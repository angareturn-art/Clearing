...
### 2.1 작업 전 백업 절차 (절대 규칙)
> [!CAUTION]
> **디비(DB) 관련 데이터 수정, 스키마 변경, 혹은 데이터 일괄 처리 스크립트 작성 및 실행 전에는 반드시 현재 상태의 DB를 물리적으로 백업해야 합니다.** (과거 데이터 유실 사고 발생 방지)
> 어떠한 경우라도 백업을 건너뛰고 DB UPDATE나 DELETE를 수행해서는 안 됩니다.

중요한 코드 수정이나 DB 변경을 시작하기 전, 다음 두 가지를 반드시 수행합니다.
1. **GitHub 동기화**: `git add .`, `git commit -m "..."`, `git push`를 통해 현재 상태를 안전하게 원격 저장소에 업로드합니다.
2. **물리적 DB 백업**: `server/construction.db` 파일을 `server/construction_backup_YYYYMMDD_HHMM.db` (시간 포함) 형식으로 현재 작업 직전 상태의 백업본을 반드시 생성합니다.
