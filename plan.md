...
### 데이터 안전 및 백업 프로토콜 (Safety First) - 절대 수칙
> **경고**: 과거 DB 백업 없이 스크립트를 실행하여 사용자가 입력한 데이터가 모두 유실된 치명적인 사고가 있었습니다. 이후 모든 AI 에이전트는 DB 관련 작업 전 반드시 아래 규칙을 강제적으로 따릅니다.

- **작업 전 GitHub 동기화**: 모든 코드 수정 전 현재 상태를 `commit & push`하여 작업 이력을 보존합니다.
- **물리적 DB 백업 (필수)**: 디비(DB) 관련 데이터 수정, 스크립트 작성/실행 전 무조건 `server/construction_backup_YYYYMMDD_HHMM.db` 형태의 백업 파일을 생성해야 합니다. 어떠한 상황에서도 예외는 없습니다.
- **데이터 무결성 원칙**: 기존 데이터는 어떠한 경우에도 삭제되어서는 안 되며, 마이그레이션 시 기존 데이터를 새로운 구조에 안전하게 이관(Mapping)해야 합니다.
...
- [x] **Phase 5: 현장별 기성비 산출 고도화** (추가 정산 로직 및 보고서 출력)
  - [x] 데이터베이스 엑셀 추출 초보자용 가이드 작성 (`ExportToExcel_Plan.md`)
  - [x] 월별 마감 화면 개발 및 공수 산정 로직 설계 (완료)
  - [x] 월별 마감 공수 산정 로직 현장 실무 기준으로 전면 교체 (2026-04-29, commit: `37ee228`)
